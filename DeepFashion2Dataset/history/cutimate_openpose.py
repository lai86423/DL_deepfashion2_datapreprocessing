...
try:
    ...
    def mask_line(point1,point2,image,thickness, save_path, img_name):

        mask = np.full(image.shape,(0,0,0), np.uint8)

        cv2.line(mask, point1, point2, (255, 255, 255), thickness=thickness)
        bitwiseAnd = cv2.bitwise_and(image, mask)
        cv2.imwrite(save_path + img_name+'_Line.jpg', bitwiseAnd)
        print("mask ok!")
                ...
                if (datum.poseKeypoints[0][1] == a).any() == False:
                    ...
                    if (datum.poseKeypoints[0][10] == a).any() == False :
                        ...
                        try:
                            ...
                            mask_line(datum.poseKeypoints[0][13][1], datum.poseKeypoints[0][12][1], img, 3, save_path, img_name)
                            if legH <=0:
                                print("!!except legH")
                                return 0
                        except Exception as e:
